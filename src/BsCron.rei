...
module CronTime: {
  ...
  /** [make(cronTime, timezone, utcOffset)] will create a new [CronTime]. This can be used with {!val:setTime} to change when a [CronJob] is fired.
...
let nextMomentDates:
  (~numberOfDates: int=?, CronJob.t) => array(MomentRe.Moment.t);
